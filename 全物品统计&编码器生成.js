...
const ScanPos = [       //起始点              附着方块位置        层          颜色(侧)
    { startPos: pos(1000460, 182, 10000586), faceOffset: "down", floor: 1 ,color: "magenta" },
    { startPos: pos(1000462, 181, 10000586), faceOffset: "west", floor: 2 ,color: "magenta" },
    { startPos: pos(1000462, 179, 10000586), faceOffset: "west", floor: 3 ,color: "magenta" },
    { startPos: pos(1000462, 177, 10000586), faceOffset: "up", floor: 4 ,color: "magenta" },
    { startPos: pos(1000450, 182, 10000586), faceOffset: "down", floor: 1 ,color: "pink" },
    { startPos: pos(1000448, 181, 10000586), faceOffset: "east", floor: 2 ,color: "pink" },
    { startPos: pos(1000448, 179, 10000586), faceOffset: "east", floor: 3 ,color: "pink" },
    { startPos: pos(1000448, 177, 10000586), faceOffset: "up", floor: 4 ,color: "pink" }
]
// const ScanPos = [       //起始点              附着方块位置        层          颜色(侧)
//     { startPos: pos(555619, 182, 10000586), faceOffset: "down", floor: 1 ,color: "orange" },
//     { startPos: pos(555619, 182, 10000586), faceOffset: "south", floor: 2 ,color: "orange" },
//     { startPos: pos(555619, 181, 10000586), faceOffset: "south", floor: 3 ,color: "orange" },
//     { startPos: pos(555619, 179, 10000586), faceOffset: "south", floor: 4 ,color: "orange" },
//     { startPos: pos(555619, 177, 10000586), faceOffset: "up", floor: 5 ,color: "orange" },
//     { startPos: pos(555619, 182, 10000586), faceOffset: "down", floor: 1 ,color: "pink" },
//     { startPos: pos(555619, 182, 10000586), faceOffset: "north", floor: 2 ,color: "pink" },
//     { startPos: pos(555619, 181, 10000586), faceOffset: "north", floor: 3 ,color: "pink" },
//     { startPos: pos(555619, 179, 10000586), faceOffset: "north", floor: 4 ,color: "pink" },
//     { startPos: pos(555619, 177, 10000586), faceOffset: "up", floor: 5 ,color: "pink" }
// ]
//=========配置==============
const SEGMENT_LENGTH = 144 // 每段长度
const MAX_LENGTH = 144   //遍历终点
const SCAN_DIRT = "south"    //遍历方向
const ChestCapacity = 54    //箱子容量
const DELAY = 10 //遍历时间间隔(ms) //只是为了好看，实际可以改成0
...
const MT_GROUP = Math.ceil(MAX_LENGTH/SEGMENT_LENGTH)

//批量获取
for(let i = 1; i <= ScanPos.length/2; i++){
    getChest("pink", "F", i)
}
for(let i = 1; i <= MT_GROUP; i++){
    getChest("pink", "MT", i)
}
...
function addItem(id){
    //Hud.clearDraw2Ds()
    //let item = ItemList.find(item => {return item.getId() == id})?.getDefaultStack()
...
function PushIn(id, count, pos, components){
    // 跳过空或无效物品
    if (!id || id === "minecraft:air" || id === "minecraft:bedrock") return
    Items.push(id)
...
function newChest(items, name){
    ...
    while(totalCount >= 0){
        if(filledCount[index] == 16){
            filledCount[index] = 2
            let count = Math.min(Math.floor(totalCount/4), 14)
...
function newItem(id){
    return ItemList.find(item => {return item.getId() == id}).getDefaultStack().getCreative().setCount(2)
}
