...
// 2025.7.21: 
// 增加群友要求的y轴范围检测, 并更改到达目标检测方式, 现在不会卡住
// 新增了xz模式, 此模式中baritone会忽略y轴直接寻路, 适用于捡天上掉下来的东西, 可能更符合群友们的使用情况
...
//================config=================//
const pathMode = "xz"; // 可选值 "xyz" 或 "xz"
...
const GoalYLevel = Java.type("baritone.api.pathing.goals.GoalYLevel");
const GoalComposite = Java.type("baritone.api.pathing.goals.GoalComposite");
...
function goPickUp(item) {
    ...
    while (goalProcess.isActive() && item.isAlive() && GlobalVars.getBoolean(scriptname)) {
        Time.sleep(50);
    }

    if (!item.isAlive()) {
        Chat.actionbar("物品已被拾取");
    } else if (!goalProcess.isActive()) {
        Chat.actionbar("Baritone停止任务, 可能已到达或放弃");
    }
}
